...
def add_student_result(value):
    ...
    print(Fore.RED + "Expected input for number of student is number, example 10, 24")
    record = int(input(Fore.GREEN + "Enter the number of students: "))
    ...
    for i in range(0, record):
        name = input(Fore.GREEN + "Enter the student name :").split()
...
def calc_student_performance():
    ...
    max_result = 98
    students = {"eva": 90, "bob": 85, "maria": 72,
                "grace": 63, "dan": 51, "eric": 43}
    students_list = []
    stud_list_of_list = []
